...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // Create a single GTFS service called S.
        CalendarDate gtfsCalendarDate = new CalendarDate();
        gtfsCalendarDate.date = LocalDate.of(2017, 1, 1);
        gtfsCalendarDate.exception_type = 1; // Add service on this date.
        gtfsCalendarDate.service_id = "S";
        Service gtfsService = new Service(gtfsCalendarDate.service_id);
        gtfsService.calendar_dates.put(gtfsCalendarDate.date, gtfsCalendarDate);
        ...
        for (TransitLine matsimLine : scenario.getTransitSchedule().getTransitLines().values()) {
            ...
            gtfsRoute.route_long_name = matsimLine.getName();
            // Routes with IDs starting with a letter are train lines. Numbers are bus lines.
            gtfsRoute.route_type = Character.isLetter(gtfsRoute.route_id.charAt(0)) ? 1 : 3;
            ...
            for (TransitRoute matsimRoute : matsimLine.getRoutes().values()) {
                // matsimRoute.getTransportMode();
                String gtfsPatternId = matsimRoute.getId().toString();
                int tripOnPattern = 0;
                for (Departure matsimDeparture : matsimRoute.getDepartures().values()) {
                    ...
                    gtfsTrip.trip_id = gtfsPatternId + "_" + tripOnPattern;
                    gtfsTrip.trip_headsign = matsimRoute.getStops().get(matsimRoute.getStops().size() - 1).getStopFacility().getName();
                    gtfsTrip.block_id = matsimDeparture.getVehicleId().toString();
                    // gtfsTrip.trip_short_name;
